...
class Services_model extends CI_Model
{
    ...
    public function get_service_payments($package_id)
    {
        $this->db->select('
            ...
            cppt.package_id,
            cppt.payment_amount,
            cppt.expected_amount,               // Now directly from client_package_payments_tbl
            cppt.payment_date,
            cppt.payment_method AS payment_transaction_method,
            cppt.payment_status AS payment_status, // Now directly from client_package_payments_tbl
            cppt.notes,
            cppt.added_on,
            cpt.status AS service_status,      // Keep original service status from client_packages_tbl
            cpt.service_name
        ');
        ...
        error_log("DEBUG SQL Query: " . $last_query); // Logs to PHP error log

        $query = $this->db->get();

        if ($query) {
            $result = $query->result_array();
            error_log("DEBUG Query Result Count: " . count($result)); // Logs number of rows
            error_log("DEBUG Query Result Data: " . json_encode($result)); // Logs actual data (be careful with sensitive data in production)
            return $result;
        } else {
            error_log("DEBUG Query Failed: " . $this->db->error()['message']); // Logs database error message if query failed
            return false;
        }
        // --- END DEBUGGING LINES ---
    }
